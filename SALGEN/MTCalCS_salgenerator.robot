*** Settings ***
Documentation    This suite builds the various interfaces for the MTCalCS.
Force Tags    salgen    
...
*** Test Cases ***
Verify MTCalCS XML Defintions exist
    [Tags]
    Comment    Verify the CSC XML definition files exist.
    ${output}    Run Process    ls     ${SALWorkDir}/MTCalCS_*.xml    shell=True
    Log Many    ${output.stdout}    ${output.stderr}
    Should Not Contain    ${output.stderr}    No such file or directory    msg="MTCalCS has no XML defintions"    values=False
    Should Not Be Empty    ${output.stdout}
    File Should Exist    ${SALWorkDir}/MTCalCS_Events.xml
    File Should Exist    ${SALWorkDir}/MTCalCS_Telemetry.xml

Salgen MTCalCS Validate
    [Documentation]    Validate the MTCalCS XML definitions.
    [Tags]    validate
...
Salgen MTCalCS HTML
    ...
    Log Many    @{files}
    File Should Exist    ${SALWorkDir}/html/${subSystem}/MTCalCS_Events.html
    File Should Exist    ${SALWorkDir}/html/${subSystem}/MTCalCS_Telemetry.html
    File Should Exist    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl

Verify MTCalCS revCodes File
    [Documentation]    Ensure MTCalCS_revCodes.tcl contains 1 revcode per topic.
    [Tags]    html    
...
Verify MTCalCS Telemetry directories
    [Tags]    cpp
...
Verify MTCalCS C++ Telemetry Interfaces
...
Verify MTCalCS C++ Event Interfaces
    [Documentation]    Verify the C++ interfaces were properly created.
    [Tags]    cpp
...
Verify MTCalCS Python Telemetry Interfaces
...
Verify MTCalCS Python Event Interfaces
    [Documentation]    Verify the Python interfaces were properly created.
    [Tags]    python
    @{files}=    List Directory    ${SALWorkDir}/${subSystem}/python    pattern=*${subSystem}*
    Log Many    @{files}
