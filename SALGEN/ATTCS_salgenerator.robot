...
Verify ATTCS XML Defintions exist
    ...
    ${output}    Run Process    ls     ${SALWorkDir}/ATTCS_*.xml    shell=True
    Log Many    ${output.stdout}    ${output.stderr}
    Should Not Contain    ${output.stderr}    No such file or directory    msg="ATTCS has no XML defintions"    values=False
...
Verify ATTCS revCodes File
    [Documentation]    Ensure ATTCS_revCodes.tcl contains 1 revcode per topic.
    [Tags]    html    
    ...
    Should Match Regexp    ${output}    set REVCODE\\(${subSystem}_command_spectrographSetup\\) [a-z0-9]{8,}
    ...
    Should Match Regexp    ${output}    set REVCODE\\(${subSystem}_logevent_spectrographInPosition\\) [a-z0-9]{8,}
    Should Match Regexp    ${output}    set REVCODE\\(${subSystem}_logevent_telescopeInPosition\\) [a-z0-9]{8,}
    ...
    Should Match Regexp    ${output}    set REVCODE\\(${subSystem}_logevent_settingsApplied_Example\\) [a-z0-9]{8,}
