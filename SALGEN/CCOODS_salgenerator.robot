*** Settings ***
Documentation    This suite builds the various interfaces for the CCOODS.
Force Tags    salgen    
...
*** Test Cases ***
Verify CCOODS XML Defintions exist
    [Tags]
    Comment    Verify the CSC XML definition files exist.
    ${output}    Run Process    ls     ${SALWorkDir}/CCOODS_*.xml    shell=True
    Log Many    ${output.stdout}    ${output.stderr}
    Should Not Contain    ${output.stderr}    No such file or directory    msg="CCOODS has no XML defintions"    values=False
    Should Not Be Empty    ${output.stdout}
    File Should Exist    ${SALWorkDir}/CCOODS_Events.xml

Salgen CCOODS Validate
    [Documentation]    Validate the CCOODS XML definitions.
    [Tags]    validate
...
Verify CCOODS revCodes File
    [Documentation]    Ensure CCOODS_revCodes.tcl contains 1 revcode per topic.
    [Tags]    html    
...
Verify CCOODS C++ Command Interfaces
...
Verify CCOODS C++ Event Interfaces
    [Documentation]    Verify the C++ interfaces were properly created.
    [Tags]    cpp
...
Verify CCOODS Python Command Interfaces
...
Verify CCOODS Python Event Interfaces
    [Documentation]    Verify the Python interfaces were properly created.
    [Tags]    python
    @{files}=    List Directory    ${SALWorkDir}/${subSystem}/python    pattern=*${subSystem}*
    Log Many    @{files}
