...
Verify MTAlignment XML Defintions exist
    ...
    ${output}    Run Process    ls     ${SALWorkDir}/MTAlignment_*.xml    shell=True
    Log Many    ${output.stdout}    ${output.stderr}
    Should Not Contain    ${output.stderr}    No such file or directory    msg="MTAlignment has no XML defintions"    values=False
...
Salgen MTAlignment HTML
    ...
    File Should Exist    ${SALWorkDir}/html/${subSystem}/MTAlignment_Commands.html
    File Should Exist    ${SALWorkDir}/html/${subSystem}/MTAlignment_Events.html
    File Should Exist    ${SALWorkDir}/html/${subSystem}/MTAlignment_Telemetry.html
    Should Contain    ${output.stdout}    Added sal-generator-${subSystem}.trackertemp to form
    @{items}=    List Directory    ${SALWorkDir}/html/salgenerator
    Directory Should Exist    ${SALWorkDir}/html/salgenerator/${subSystem}
    @{files}=    List Directory    ${SALWorkDir}/html/salgenerator/${subSystem}    pattern=*${subSystem}*
    Log Many    @{files}
    File Should Exist    ${SALWorkDir}/html/salgenerator/${subSystem}/MTAlignment_trackertemp-metadata.html
    File Should Exist    ${SALWorkDir}/html/salgenerator/${subSystem}/MTAlignment_trackertemp-streamdef.html
...
Salgen MTAlignment C++
    ...
    Should Contain    ${output.stdout}    SAL generator - ${SALVersion}
    Should Contain    ${output.stdout}    Generating SAL CPP code for ${subSystem}_trackertemp.idl
...
Verify MTAlignment Telemetry directories
    [Tags]    cpp
    @{files}=    List Directory    ${SALWorkDir}    pattern=*${subSystem}*
    Log Many    @{files}
    Directory Should Exist    ${SALWorkDir}/${subSystem}_trackertemp

Verify MTAlignment C++ Telemetry Interfaces
    [Documentation]    Verify the C++ interfaces were properly created.
    [Tags]    cpp
    File Should Exist    ${SALWorkDir}/${subSystem}_trackertemp/cpp/standalone/sacpp_${subSystem}_pub
    File Should Exist    ${SALWorkDir}/${subSystem}_trackertemp/cpp/standalone/sacpp_${subSystem}_sub

Verify MTAlignment C++ Command Interfaces
    [Documentation]    Verify the C++ interfaces were properly created.
    [Tags]    cpp
    ...
    File Should Exist    ${SALWorkDir}/${subSystem}/cpp/src/sacpp_${subSystem}_laserPower_controller
    File Should Exist    ${SALWorkDir}/${subSystem}/cpp/src/sacpp_${subSystem}_healthCheck_commander
    File Should Exist    ${SALWorkDir}/${subSystem}/cpp/src/sacpp_${subSystem}_healthCheck_controller

Verify MTAlignment C++ Event Interfaces
    [Documentation]    Verify the C++ interfaces were properly created.
    [Tags]    cpp
    File Should Exist    ${SALWorkDir}/${subSystem}/cpp/src/sacpp_${subSystem}_offsetsPublish_send
    File Should Exist    ${SALWorkDir}/${subSystem}/cpp/src/sacpp_${subSystem}_offsetsPublish_log
    File Should Exist    ${SALWorkDir}/${subSystem}/cpp/src/sacpp_${subSystem}_positionPublish_send
    File Should Exist    ${SALWorkDir}/${subSystem}/cpp/src/sacpp_${subSystem}_positionPublish_log
    File Should Exist    ${SALWorkDir}/${subSystem}/cpp/src/sacpp_${subSystem}_t2saError_send
    File Should Exist    ${SALWorkDir}/${subSystem}/cpp/src/sacpp_${subSystem}_t2saError_log

Salgen MTAlignment Python
...
Verify MTAlignment Python Telemetry Interfaces
    [Documentation]    Verify the Python interfaces were properly created.
    [Tags]    python
    @{files}=    List Directory    ${SALWorkDir}/${subSystem}/python    pattern=*${subSystem}*
    Log Many    @{files}
    File Should Exist    ${SALWorkDir}/${subSystem}/python/${subSystem}_trackertemp_Publisher.py
    File Should Exist    ${SALWorkDir}/${subSystem}/python/${subSystem}_trackertemp_Subscriber.py

Verify MTAlignment Python Command Interfaces
    [Documentation]    Verify the Python interfaces were properly created.
    [Tags]    python
    @{files}=    List Directory    ${SALWorkDir}/${subSystem}/python    pattern=*${subSystem}*
    Log Many    @{files}
    ...
    File Should Exist    ${SALWorkDir}/${subSystem}/python/${subSystem}_Controller_laserPower.py
    File Should Exist    ${SALWorkDir}/${subSystem}/python/${subSystem}_Commander_healthCheck.py
    File Should Exist    ${SALWorkDir}/${subSystem}/python/${subSystem}_Controller_healthCheck.py

Verify MTAlignment Python Event Interfaces
    [Documentation]    Verify the Python interfaces were properly created.
    [Tags]    python
    @{files}=    List Directory    ${SALWorkDir}/${subSystem}/python    pattern=*${subSystem}*
    Log Many    @{files}
    File Should Exist    ${SALWorkDir}/${subSystem}/python/${subSystem}_Event_offsetsPublish.py
    File Should Exist    ${SALWorkDir}/${subSystem}/python/${subSystem}_EventLogger_offsetsPublish.py
    File Should Exist    ${SALWorkDir}/${subSystem}/python/${subSystem}_Event_positionPublish.py
    File Should Exist    ${SALWorkDir}/${subSystem}/python/${subSystem}_EventLogger_positionPublish.py
    File Should Exist    ${SALWorkDir}/${subSystem}/python/${subSystem}_Event_t2saError.py
    File Should Exist    ${SALWorkDir}/${subSystem}/python/${subSystem}_EventLogger_t2saError.py

Salgen MTAlignment LabVIEW
...
Salgen MTAlignment Java
    ...
    Log Many    ${output.stdout}    ${output.stderr}
    Should Contain    ${output.stdout}    SAL generator - ${SALVersion}
    Should Contain    ${output.stdout}    Generating SAL Java code for ${subSystem}_trackertemp.idl
