*** Settings ***
Documentation    This suite builds the various interfaces for the IOTA.
Force Tags    salgen    
...
*** Test Cases ***
Verify IOTA XML Defintions exist
    [Tags]
    Comment    Verify the CSC XML definition files exist.
    ${output}    Run Process    ls     ${SALWorkDir}/IOTA_*.xml    shell=True
    Log Many    ${output.stdout}    ${output.stderr}
    Should Not Contain    ${output.stderr}    No such file or directory    msg="IOTA has no XML defintions"    values=False
    Should Not Be Empty    ${output.stdout}
    File Should Exist    ${SALWorkDir}/IOTA_Events.xml
    File Should Exist    ${SALWorkDir}/IOTA_Telemetry.xml

Salgen IOTA Validate
    [Documentation]    Validate the IOTA XML definitions.
    [Tags]    validate
...
Salgen IOTA HTML
    ...
    File Should Exist    ${SALWorkDir}/html/${subSystem}/IOTA_Events.html
    File Should Exist    ${SALWorkDir}/html/${subSystem}/IOTA_Telemetry.html
    ...
    Log Many    @{files}
    File Should Exist    ${SALWorkDir}/html/salgenerator/${subSystem}/IOTA_timestamp-metadata.html
    File Should Exist    ${SALWorkDir}/html/salgenerator/${subSystem}/IOTA_timestamp-streamdef.html
    File Should Exist    ${SALWorkDir}/html/salgenerator/${subSystem}/IOTA_loopTime-metadata.html
    File Should Exist    ${SALWorkDir}/html/salgenerator/${subSystem}/IOTA_loopTime-streamdef.html
    File Should Exist    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl

Verify IOTA revCodes File
    [Documentation]    Ensure IOTA_revCodes.tcl contains 1 revcode per topic.
    [Tags]    html    
...
Verify IOTA Telemetry directories
    [Tags]    cpp
...
Verify IOTA C++ Telemetry Interfaces
...
Verify IOTA C++ Event Interfaces
    [Documentation]    Verify the C++ interfaces were properly created.
    [Tags]    cpp
...
Verify IOTA Python Telemetry Interfaces
...
Verify IOTA Python Event Interfaces
    [Documentation]    Verify the Python interfaces were properly created.
    [Tags]    python
    @{files}=    List Directory    ${SALWorkDir}/${subSystem}/python    pattern=*${subSystem}*
    Log Many    @{files}
