...
function salgenRPM() {
    ...
    echo "    \${output}=    Run Process    \${SALHome}/bin/salgenerator    \${subSystem}    rpm    version\\=\${SALVersion}\${Build_Number}    \
shell=True    cwd=\${SALWorkDir}    stdout=\${EXECDIR}\${/}\${subSystem}_stdout.txt    stderr=\${EXECDIR}\${/}\${subSystem}_stderr.txt" >> $testSuite
    echo "    Log Many    \${output.stdout}    \${output.stderr}" >> $testSuite
    ...
    echo "    File Should Exist    \${SALWorkDir}/rpmbuild/RPMS/x86_64/ts_sal_runtime-\${XMLVersion}_\${SALVersion}\${Build_Number}\${DIST}.x86_64.rpm" >> $testSuite
    echo "    File Should Exist    \${SALWorkDir}/rpmbuild/RPMS/x86_64/ts_sal_ATruntime-\${XMLVersion}_\${SALVersion}\${Build_Number}\${DIST}.x86_64.rpm" >> $testSuite
    echo "    File Should Exist    \${SALWorkDir}/rpmbuild/RPMS/x86_64/ts_sal_utils-\${SALVersion}-1.x86_64.rpm" >> $testSuite
    echo "    File Should Exist    \${SALWorkDir}/rpmbuild/RPMS/x86_64/\${subSystem}-\${XMLVersion}_\${SALVersion}\${Build_Number}\${DIST}.x86_64.rpm" >> $testSuite
    echo "    File Should Exist    \${SALWorkDir}/rpmbuild/RPMS/x86_64/\${subSystem}_test-\${XMLVersion}_\${SALVersion}\${Build_Number}\${DIST}.x86_64.rpm" >> $testSuite
    echo "" >> $testSuite
}
