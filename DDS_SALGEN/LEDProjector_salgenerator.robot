*** Settings ***
Documentation    This suite builds the various interfaces for the LEDProjector.
Force Tags    salgen    ledprojector
...
*** Variables ***
${subSystem}    LEDProjector
${timeout}    1200s

*** Test Cases ***
Verify LEDProjector XML Defintions exist
    [Tags]
    Comment    Verify the CSC XML definition files exist.
    ${output}    Run Process    ls     ${SALWorkDir}/${subSystem}_*xml    shell=True
    Log Many    ${output.stdout}    ${output.stderr}
    Should Not Be Empty    ${output.stdout}
    File Should Exist    ${SALWorkDir}/LEDProjector_Commands.xml
    File Should Exist    ${SALWorkDir}/LEDProjector_Events.xml

Salgen LEDProjector Validate
    [Documentation]    Validate the LEDProjector XML definitions.
    [Tags]    validate
    ...
    File Should Exist    ${SALWorkDir}/idl-templates/${subSystem}_command_switchAllOn.idl
    File Should Exist    ${SALWorkDir}/idl-templates/${subSystem}_command_switchAllOff.idl
    File Should Exist    ${SALWorkDir}/idl-templates/${subSystem}_command_switchOn.idl
    File Should Exist    ${SALWorkDir}/idl-templates/${subSystem}_command_switchOff.idl
    File Should Exist    ${SALWorkDir}/idl-templates/${subSystem}_command_adjustDACPower.idl
    File Should Exist    ${SALWorkDir}/idl-templates/${subSystem}_command_adjustAllDACPower.idl
    ...
    File Should Exist    ${SALWorkDir}/idl-templates/${subSystem}_logevent_ledState.idl

Verify LEDProjector revCodes File
    [Documentation]    Ensure LEDProjector_revCodes.tcl contains 1 revcode per topic.
    [Tags]    doc    
    ...
    Should Match Regexp    ${output}    set REVCODE\\(${subSystem}_command_switchAllOn\\) [a-z0-9]{8,}
    Should Match Regexp    ${output}    set REVCODE\\(${subSystem}_command_switchAllOff\\) [a-z0-9]{8,}
    Should Match Regexp    ${output}    set REVCODE\\(${subSystem}_command_switchOn\\) [a-z0-9]{8,}
    Should Match Regexp    ${output}    set REVCODE\\(${subSystem}_command_switchOff\\) [a-z0-9]{8,}
    Should Match Regexp    ${output}    set REVCODE\\(${subSystem}_command_adjustDACPower\\) [a-z0-9]{8,}
    Should Match Regexp    ${output}    set REVCODE\\(${subSystem}_command_adjustAllDACPower\\) [a-z0-9]{8,}
    ...
    Should Match Regexp    ${output}    set REVCODE\\(${subSystem}_logevent_ledState\\) [a-z0-9]{8,}

Salgen LEDProjector IDL
...
Verify LEDProjector RPM Contents
